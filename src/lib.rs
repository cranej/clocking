...
pub trait ClockingStore {
    fn start_clocking(&mut self, item: &ClockingItem) -> bool;
    fn finish_clocking(&mut self, id: &ClockingItemId, end: &DateTime<Utc>, notes: &str) -> bool;
    fn query_clocking(&self, start: &DateTime<Utc>, end: Option<DateTime<Utc>>) -> Vec<ClockingItem>;
}

pub struct MemoryStore {
    items: Vec<ClockingItem>,
}
...
impl ClockingStore for MemoryStore {
    fn start_clocking(&mut self, item: &ClockingItem) -> bool {
        match self.items.iter().find(|x| x.id == item.id) {
            Some(_) => { println!("Existed..."); false },
            None => { self.items.push(item.clone()); true },
        }
    }
    ...
    fn finish_clocking(&mut self, id: &ClockingItemId, end: &DateTime<Utc>, notes: &str) -> bool {
        match self.items.iter().position(|x| x.id == *id) {
            Some(pi)  => {
                let mut item = &mut self.items[pi];
                if item.end.is_some() {
                    println!("Try to finish already ended item.");
                    false
                } else if item.id.start >= *end {
                    println!("End before start.");
                    false
                } else {
                    item.end = Some(end.clone());
                    item.notes.push_str(notes);
                    true
                }
            },
            None => {println!("Item does not exist."); false },
        }
    }
        ...
        self.items.iter().filter(|x| {
            x.id.start >= *start &&
                (match (x.end, end) {
                    (None, _) => false,
                    (_, None) => true,
                    (Some(xend), Some(end)) => xend <= end,
                })
        }).map(|x| x.clone()).collect::<Vec<ClockingItem>>()
    }
}
...
mod tests {
    ...
    fn sqlite_store_basic_workflow() {
        let mut mem_store = SqliteStore::new(":memory:");
        ...
        assert_eq!(mem_store.start_clocking(&item), true);
        // add again
        assert_eq!(mem_store.start_clocking(&item), false, "Adding the same item twice should fail.");
        ...
        let end = Utc::now();
        let note = "A note";
        assert_eq!(mem_store.finish_clocking(&item.id, &end, &note), true);
        //finish again
        assert_eq!(mem_store.finish_clocking(&item.id, &end, &note), false, "call finish_clocking on finished item should fail");
    ...
    fn sqlite_store_query() {
        ...
        for item in items.iter() {
            assert!(mem_store.start_clocking(item));
        }

        // ends item1, item3, and item4
        // after the finish_clocking calls, data should be like:
        //  0. @now - 4d5min, None
        ...
        for(end_item, end_time) in end_data.iter() {
            assert!(mem_store.finish_clocking(&end_item.id, &end_time, add_note));
        }
